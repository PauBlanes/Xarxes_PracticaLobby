...
void DBManager::CloseSession(int idSession) {
	
	string cmd = "UPDATE Sessions SET end = current_timestamp WHERE idSession =" + to_string(idSession); //NO RECORDO SI EL NOM DE LA COLUMNA ERA EXACTE numGames
	stmt->execute(cmd.c_str());
}